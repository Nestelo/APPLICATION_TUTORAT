...
const StudentMessagesScreen = ({ navigation }) => {
  ...
  const [filters] = useState([
    ...
    { id: 'tuteurs', label: 'Tuteurs', icon: 'school-outline' },
  ...
  const loadAvailableUsers = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        // Filtrer pour n'afficher que les tuteurs et admin
  ...
  const handleStartConversation = async (userId) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/messagerie/conversations/start/`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const getRoleIcon = (role) => {
    switch (role) {
      case 'tuteur': return 'school-outline';
      case 'admin': return 'shield-outline';
      case 'enseignant': return 'book-outline';
  ...
  const getRoleColor = (role) => {
    switch (role) {
      case 'tuteur': return '#007bff';
      case 'admin': return '#dc3545';
      case 'enseignant': return '#28a745';
      ...
      >
        <View style={styles.modalContainer}>
              ...
              <FlatList
                ...
                ListHeaderComponent={
                  <Text style={styles.usersListHeader}>
                    Choisissez un tuteur ou un administrateur
                  </Text>
                }
              />
            </View>
          ...
          <View style={styles.modalActions}>
            ...
            >
              <Text style={styles.createButtonText}>Commencer</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
